...
for subject_idx = 1:Number_of_subjects
    current_dir_name = strcat(eeg_file_folder,'Subject',num2str(subject_idx),'/');
    current_dir = dir(current_dir_name);
    for file_idx = 1:Number_of_gdf_per_person
        EEG{subject_idx, file_idx} = pop_biosig(strcat(current_dir_name,current_dir(file_idx+2).name));
    end
end
